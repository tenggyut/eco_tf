...
class InceptionI3d(snt.AbstractModule):
  ...
  def get_finetunning(self, inputs, pretrain_ckpt_path, is_training, dropout_keep_prob=1.0):
      net, _ = self._build(inputs, is_training, dropout_keep_prob)
      if pretrain_ckpt_path:
          self.load_pretrain(pretrain_ckpt_path)
      with tf.variable_scope('finetunning_logits'):
          ...
          pred_prob = tf.nn.softmax(averaged_logits)
          pred_labels = tf.argmax(pred_prob, -1, name = 'pred_labels')
     return averaged_logits, {'pred_probs': pred_prob, 'pred_labels':pred_labels}
  ...
  def load_pretrain(self, pretrain_ckpt_path):
      rgb_variable_map = {}
      ...
      tf.train.init_from_checkpoint(pretrain_ckpt_path, rgb_variable_map)
